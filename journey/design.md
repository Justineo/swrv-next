...
- The same `core-cache-provider` domain file now also covers public `SWRVConfig.defaultValue` exposure plus custom `initFocus` and `initReconnect` hooks that own provider-scoped event listeners without replacing the inherited cache by default.
