...
- A dedicated `core-local-mutate` domain file now covers bound, scoped, and global mutate behavior, including optimistic local updates, `rollbackOnError: false`, committed-snapshot `populateCache` transforms, function-style `revalidate`, filtered mutate behavior, internal-key filtering, shared local state without a fetcher, scoped mutate callback inputs, null-key global mutate behavior, unmounted-key dedupe invalidation, in-flight request and mutation suppression, local mutation error isolation, and bound revalidation without clearing current data first.
